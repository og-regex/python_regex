...
#findall() finds alt substrings where the RE matches returning them as a list 
...
#use end() to return the ending posttton of the match 
...
#Now well use a different string to match
p = re.compile(r'\d+')
...
# match() determines wheter the RE matches at the beginning of the string
m = p.match(s)
...
#There are two pattern methods that return all of the matches for a patter
...
#finditer() reuturns a sequence of matching objects instances as an iterator
iter = p.finditer(s)
